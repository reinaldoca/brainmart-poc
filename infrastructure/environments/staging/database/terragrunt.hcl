...
locals {
  _sg_raw     = run_cmd("--terragrunt-quiet", "sh", "-c",
    "aws ec2 describe-security-groups --filters 'Name=group-name,Values=brainmart-staging-rds-sg' --region us-east-1 --query 'SecurityGroups[0].GroupId' --output text 2>/dev/null || echo ''")
