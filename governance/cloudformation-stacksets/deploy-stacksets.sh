...
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# deploy-stacksets.sh
# Orquestador de CloudFormation StackSets para la organizaci??n Brainmart
#
# PROP??SITO: Despliega los 3 StackSets de Capa 0 en el orden correcto:
#   1. IAM Roles (prerequisito para los dem??s)
...
#   - La cuenta debe ser la Master de la organizaci??n AWS
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --help|-h)
      ...
      echo "  AWS_MASTER_ACCOUNT_ID         ID de la cuenta master de la organizaci??n"
      echo "  AWS_SHARED_SERVICES_ACCOUNT_ID  ID de la cuenta de CI/CD"
      echo "  AWS_ORG_ID                    ID de la organizaci??n (o-xxxxxxxxxx)"
      echo "  AWS_ROOT_OU_ID                ID del OU ra??z (r-xxxx)"
...
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# FUNCI??N: Validar prerrequisitos
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
validate_prerequisites() {
  ...
  if ! command -v aws &> /dev/null; then
    log_error "AWS CLI no est?? instalado. Instalar: https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html"
    exit 1
  fi
  log_success "AWS CLI instalado: $(aws --version 2>&1 | head -1)"

  # Verificar que jq est?? instalado (necesario para parsear respuestas JSON)
  if ! command -v jq &> /dev/null; then
    log_error "jq no est?? instalado. Instalar: brew install jq"
    exit 1
  fi
  ...
  if [[ "$org_feature_set" != "ALL" ]]; then
    log_error "La organizaci??n AWS no tiene 'All Features' habilitadas."
  ...
  # Verificar que el bucket de auditor??a existe (o crearlo)
  if ! aws s3api head-bucket --bucket "$AUDIT_BUCKET_NAME" 2>/dev/null; then
    log_warning "Bucket de auditor??a no existe: $AUDIT_BUCKET_NAME"
    if [[ "$DRY_RUN" == "false" ]]; then
      create_audit_bucket
    else
      log_info "[DRY-RUN] Se crear??a el bucket: $AUDIT_BUCKET_NAME"
    fi
  else
    log_success "Bucket de auditor??a existe: $AUDIT_BUCKET_NAME"
  fi
}

# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# FUNCI??N: Crear bucket de auditor??a centralizado
# Este bucket recibe logs de CloudTrail, Config y GuardDuty de TODAS las cuentas
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
create_audit_bucket() {
  log_info "Creando bucket de auditor??a centralizado: $AUDIT_BUCKET_NAME"
  ...
  # Bloquear acceso p??blico (no puede ser p??blico nunca)
  ...
  # Pol??tica del bucket: permitir que CloudTrail de TODA la organizaci??n escriba aqu??
  ...
  # Lifecycle: mover a Glacier despu??s de 1 a??o, eliminar despu??s de 7 a??os
  ...
  log_success "Bucket de auditor??a creado y configurado: $AUDIT_BUCKET_NAME"
}

# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# FUNCI??N: Habilitar confianza de servicios en Organizations
# Necesario para que StackSets se desplieguen autom??ticamente en nuevas cuentas
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
enable_trusted_access() {
  ...
  for service in "${services[@]}"; do
    ...
    if [[ "$DRY_RUN" == "false" ]]; then
      ...
      log_info "[DRY-RUN] Se habilitar??a trusted access para: $service"
    fi
  done

  # Habilitar StackSets de confianza (para despliegue autom??tico en OU)
  if [[ "$DRY_RUN" == "false" ]]; then
...
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# FUNCI??N: Desplegar un StackSet
# $1: nombre del StackSet
# $2: ruta al template
# $3: par??metros en formato JSON
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
deploy_stackset() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    log_info "[DRY-RUN] Se desplegar??a el StackSet: $stackset_name desde $template_file"
    return 0
  fi
  ...
  if aws cloudformation describe-stack-set --stack-set-name "$stackset_name" &>/dev/null; then
    ...
    # Desplegar en todas las cuentas de la organizaci??n bajo el OU ra??z
...
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# FUNCI??N: Esperar a que las operaciones del StackSet terminen
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
wait_for_stackset_operation() {
  local stackset_name="$1"
  local max_wait=600  # 10 minutos m??ximo
  local elapsed=0
  local interval=15
  ...
  while [[ $elapsed -lt $max_wait ]]; do
    ...
    case "$operation_status" in
      SUCCEEDED)
        log_success "StackSet $stackset_name: operaci??n completada exitosamente"
        return 0
        ;;
      FAILED|STOPPED)
        log_error "StackSet $stackset_name: operaci??n fall?? con estado $operation_status"
...
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# FUNCI??N: Validar el resultado del despliegue
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
validate_deployment() {
...
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# MAIN ??? Ejecuci??n principal
# ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
main() {
  log_section "???? Brainmart - Deploy de CloudFormation StackSets"
  log_info "Organizaci??n: $ORG_ID"
  log_info "OU Ra??z: $ROOT_OU_ID"
  log_info "Regi??n Primaria: $PRIMARY_REGION"
  log_info "Regi??n DR: $DR_REGION"
  [[ "$DRY_RUN" == "true" ]] && log_warning "MODO DRY-RUN: no se realizar??n cambios reales"
  ...
  deploy_stackset \
    ...
    "SCPs que previenen configuraciones inseguras en toda la organizaci??n"
  ...
  deploy_stackset \
    ...
    "AWS Config Rules, GuardDuty y CloudTrail en toda la organizaci??n Brainmart"

  # PASO 6: Validar el despliegue
  log_section "???? Validaci??n Final"
  validate_deployment
  ...
  log_info "  1. Verificar en la consola de AWS Organizations que las SCPs est??n activas"
  log_info "  2. Verificar en AWS Config que las Rules est??n evaluando recursos"
  log_info "  3. Verificar en GuardDuty que el detector est?? activo"
